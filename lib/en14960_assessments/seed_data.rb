...
module En14960Assessments
  # This module provides field mappings for EN14960 assessments
  ...
  module SeedData
    def self.check_passed?(inspection_passed)
      ...
      # When inspection failed, have a higher chance of individual checks failing
      rand < 0.7
    end
    ...
    def self.fan_fields(passed: true)
      {
        ...
        blower_finger_pass: passed ? true : false,  # Always fail when passed: false
        ...
        pat_comment: if passed
                       valid_date = (Date.current + 6.months).strftime("%B %Y")
                       "PAT test valid until #{valid_date}"
                     else
                       "PAT test overdue"
                     end
      }
    ...
    def self.user_height_fields(passed: true)
      {
        ...
        play_area_length_comment: "Effective play area after obstacles",
    ...
    def self.slide_fields(passed: true)
      ...
      # Calculate required runout based on platform height
      # Formula: 0.866 + 0.20 * platform_height
      ...
      runout = if passed
        ...
        (required_runout - fail_margin).round(1)
      end

      {
        ...
        runout_pass: passed ? true : false,  # Always fail runout when passed: false
        slip_sheet_pass: check_passed?(passed),
        slide_permanent_roof: false,
        slide_platform_height_comment: if passed
                                         "Platform height compliant with EN 14960"
                                       else
                                         "Platform height exceeds limits"
                                       end,
    ...
    def self.enclosed_fields(passed: true)
      {
        ...
        exit_number_pass: passed ? true : false,  # Always fail when passed: false
